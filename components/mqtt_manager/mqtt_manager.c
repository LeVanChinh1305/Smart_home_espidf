...
extern bool r1_on, r2_on, r3_on, r4_on; // Biến toàn cục lưu trạng thái Relay (được định nghĩa ở main.c)
extern TaskHandle_t mqtt_task_handle; // Biến toàn cục để lưu handle của Task MQTT Telemetry (được định nghĩa ở main.c)
        ...
        case MQTT_EVENT_DATA:
            ...
            if (json != NULL) {
                ...
                if (cJSON_IsNumber(relay_item) && cJSON_IsNumber(state_item)) {
                    ...
                    if (relay_num == 1) {
                        r1_on = relay_state; // Cập nhật trạng thái Relay 1
                        relay_ch1_set(r1_on); // Cập nhật Relay 1 thực tế
                        ESP_LOGI(TAG, "-> Đã %s Relay 1 qua MQTT", relay_state ? "BẬT" : "TẮT");
                    } else if (relay_num == 2) {
                        r2_on = relay_state; // Cập nhật trạng thái Relay 2
                        relay_ch2_set(r2_on); // Cập nhật Relay 2 thực tế
                        ESP_LOGI(TAG, "-> Đã %s Relay 2 qua MQTT", relay_state ? "BẬT" : "TẮT");
                    } else if (relay_num == 3) {
                        r3_on = relay_state; // Cập nhật trạng thái Relay 3
                        relay_ch3_set(r3_on); // Cập nhật Relay 3 thực tế
                        ESP_LOGI(TAG, "-> Đã %s Relay 3 qua MQTT", relay_state ? "BẬT" : "TẮT");
                    } else if (relay_num == 4) {
                        r4_on = relay_state; // Cập nhật trạng thái Relay 4
                        relay_ch4_set(r4_on); // Cập nhật Relay 4 thực tế
                        ESP_LOGI(TAG, "-> Đã %s Relay 4 qua MQTT", relay_state ? "BẬT" : "TẮT");
                    } else {
                        ESP_LOGW(TAG, "Số Relay không hợp lệ trong lệnh MQTT!");
                    }
                    if (mqtt_task_handle != NULL) {
                        xTaskNotifyGive(mqtt_task_handle); // Bắt hàm telemetry chạy ngay để web thấy status thay đổi
                    }
                } else {
...
void mqtt_publish_sensor_data(float temp, float hum, int gas, float light, bool r1, bool r2, bool r3, bool r4) {
    if (client == NULL) return;
    char payload[256]; 
    snprintf(
        payload, sizeof(payload), 
        "{\"temp\":%.1f, \"hum\":%.1f, \"gas\":%d, \"light\":%.1f, \"r1\":%d, \"r2\":%d, \"r3\":%d, \"r4\":%d}", 
        temp, hum, gas, light, r1 ? 1 : 0, r2 ? 1 : 0, r3 ? 1 : 0, r4 ? 1 : 0);
